...
**kernel** (lowercase):
One person's instance of the model ("your kernel"). Self-describing: readable by an agent that has never seen this person or this system before. Capitalized Human Kernel refers to the project and protocol.
...
**context library**:
A person's private, human-first collection of raw and curated context — notes, logs, transcripts, files — that feeds their kernel. Messy by design; it is never required to be agent-legible.
...
**Human API**:
The permissioned interface through which another person, product, or agent queries or invokes a selected slice of someone's kernel.
