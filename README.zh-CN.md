...
Human Kernel 以 kernel 为中心：它是可读、可纠正、归你所有的恒定结构。上下文库环绕它，另一面从它向外泛化，最外层的 Human API 只开放你授权的切片。
...
| **另一面（alter ego）** | 由内核及其背后的上下文向外泛化出的表达层：像你一样回答、推理、提醒、准备、行动——不是「挂上你的数据的通用助手」。 | 只在你授予的权限内行动。 |
