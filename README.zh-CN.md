...
> **在现实世界，一张身份证让社会认识你。**
> **在 Agent 世界，什么都没有——直到现在。**
> **Human Kernel 是你的身份层：结构化、归你所有、任何你授权的 Agent 都可读。**

Agent 越来越能干，但它们并不真正知道**自己在为谁行动**。今天，「关于你」的理解散落在聊天记录和平台拥有的封闭记忆功能里——碎片化、不可携带、不可审视。

Human Kernel 选择相反的立场：一个结构化、证据支撑的个人模型——身份、价值观、关系、知识、经历、偏好、能力与授权边界——完全归你所有，任何授权 Agent 零前置即可读懂。

简单说：
- **没有 Human Kernel：**每个 Agent 从零开始猜你是谁
- **有了 Human Kernel：**任何 Agent 第一秒就知道该怎么对你

身份证证明你*是*谁。Human Kernel 让 Agent 理解你是*怎样*的人、你如何做决策、你在乎什么。它是 Agent 时代的第一个人类表示层。

## 四层结构

Human Kernel 以你的 kernel 为中心：它是可读、可纠正、归你所有的恒定结构。上下文库环绕它，一个或多个「另一面」从它向外表达，寄居于可替换的 Agent 框架；最外层的 Human API 只开放你授权的切片。内核是你的，身体是租的。

四层同心结构（也可以从最私密到最公开来读）：

| 层 | 是什么 | 你控制什么 |
| --- | --- | --- |
| **上下文库（context library）** | 环绕内核的私有全量上下文——笔记、日志、转写、文件，经年累月。乱是它的天性；它永远不需要对 agent 可读。 | 它永远不离家。内核只向它指回。 |
| **内核（kernel）** | 最中心：结构化、证据支撑的个人模型。你在 Agent 世界的身份：你是谁、你看重什么、你如何做决策、你认识谁、你能做什么。其中每一条判断都能回答：*你为什么这么认为？证据来自哪里？现在还成立吗？我能改吗？* | 存储在本地，完全归你。可携带到任何 Agent 或平台。 |
| **另一面（alter ego）** | 由内核及其背后的上下文向外泛化出的表达层：每个「另一面」都能借可替换的 Agent 框架，像你一样回答、推理、提醒、准备、行动——不是「挂上你的数据的通用助手」。 | 只在你授予的权限内行动；一个内核可以有多个「另一面」。 |
| **Human API** | 最外层的权限调用面，让其他人、产品和 agent 可以查询或调用你的某个切片。 | 哪些切片存在、谁能调用、什么需要确认，都由你决定。 |
...
- **可携带、跨平台。** 你的内核可以和任何 Agent、任何运行时、任何平台配合工作。它不被锁在任何一个 AI 厂商的生态里。
...
- ❌ 不是另一个「AI 记忆」功能。记忆功能归平台所有，锁在各自生态里。Human Kernel 归你所有，在任何地方都能用。

## 路线图

**v0.1 —— 内核成形**

- [ ] **协议 spec** —— 分区、必填字段、自举入口文件、隐私分级、变更账本 → [草案](./spec/kernel-v0.1.md)（英文）· [示例内核](./examples/ada-lumen/KERNEL.md)
- [ ] **校验器** —— 结构检查与指针完整性
